...
AutoReq, üç katmanlı bir boru hattı olarak tasarlanmıştır: ham metnin yapısal çözümlemesini üstlenen NLP ön işleme katmanı, mantıksal denetim ve stil iyileştirmesini yürüten LLM tabanlı analiz katmanı ve doğrulanmış veriyi mühendislik belgelerine dönüştüren dinamik çıktı üretim katmanı. Veri akışı tek yönlüdür; katmanlar arasındaki sınır arayüzü `core/models.py` içinde tanımlı veri sınıflarıyla sabitlenmiştir ve bir önceki katmanın üretmediği bir alan sonraki katmana taşınmaz. Bu kısıt, hata yayılımını önemli ölçüde azaltır. Tüm katmanlar Python ile gerçeklenmiş, orkestrasyon ise Streamlit tabanlı tek sayfa bir arayüzde `process_text()` zinciri üzerinden kurgulanmıştır.
...
İlk katman, müşteriden gelen ham metni Stanza Türkçe modeli üzerinden tokenizasyon, çoklu sözcük belirteçleme (Multi-Word Tokenization, MWT), POS etiketleme, lemmatizasyon ve adlandırılmış varlık tanıma (Named Entity Recognition, NER) adımlarından geçirir. Yaklaşık 150 MB boyutundaki Stanza modeli, `@st.cache_resource` ile RAM'e bir kez yüklenerek yeniden çalıştırma sürelerini saniyeler mertebesinden milisaniyelere indirir. Cümle sınırları doğal dil sınırı bulucu tarafından belirlenir; her cümle sistemde bir `Requirement` nesnesine dönüştürülür ve `REQ_001` formatında benzersiz bir kimlik alır. Aktör çıkarımı, kural tabanlı bir lemma tablosu üzerinden çalışır: {kullanıcı, sistem, yönetici, admin, müşteri} kümesindeki lemmalardan herhangi biri cümle içinde geçtiğinde ilgili aktör listesine eklenir; aynı yöntem iş nesneleri için ayrı bir lemma kümesiyle ({şifre, form, e-posta, sepet, hesap, …}) tekrarlanır. Lemma düzeyinde çalışmak, çekim eklerinin yarattığı varyasyonları ("kullanıcılar", "kullanıcının") tek bir varlığa indirgeyerek çoğul kayıt oluşumunu engeller. Stanza modelinin yüklenemediği durumlar için yedek bir alt-dize eşleştirme mekanizması devreye girer ve sistem hiçbir koşulda ham metnin başında çökmez. FR/NFR sınıflandırması ise anahtar kelime ve bağımlılık örüntülerini birleştiren sezgisel bir denetleyiciyle yapılır; performans, güvenlik, gecikme ve SSL gibi NFR sözcükleri eşleştiğinde cümle `NON_FUNCTIONAL`, aksi halde `FUNCTIONAL` olarak etiketlenir. Geliştirme sırasında "kullanıcı" ve "şifre" gibi terimlerin yarattığı yanlış pozitiflerin somut bir sorun oluşturduğu görülmüş ve bu durum Bölüm IV'teki değerlendirme bulgularında açıkça raporlanmıştır. Birinci katmanın çıktısı, alt katmanlara aktarılan tek tipte bir `ParsedDocument` nesnesidir; bu nesnede her gereksinim için aktör, nesne, tip, lemma listesi ve POS etiketleri tek bir veri sınıfında bir araya gelir.

Yapılandırılmış bu çıktı, ikinci katmanın girdisini oluşturur. `ParsedDocument` nesnesi önce metinsel bir bloğa serileştirilir; her gereksinim `[REQ_NNN] (TIP) METIN` satırı biçiminde temsil edilir ve iki parçalı bir prompta yerleştirilir. Sistem mesajı analiz personasını ve görev tanımını içerirken, kullanıcı mesajı somut gereksinim listesiyle beklenen JSON şemasını barındırır. JSON şeması `conflicts`, `gaps` ve `improvements` adlı üç dizi alanı tanımlar; her dizi `req_ids`, `conflict_type`, `severity` ve `reason` gibi alt alanlar içerir. Sistem mesajı, modelden açıklama veya markdown çitleri olmadan yalnızca geçerli bir JSON nesnesi döndürmesini ister. Çağrı Google Gemini 3.0 Flash modeline gönderilir; yanıt zaman zaman markdown kod blokları ya da açıklayıcı metinle döndüğünden, ham çıktı `extract_json_object()` adlı yardımcı fonksiyondan geçirilerek ilk geçerli JSON nesnesi izole edilir. Bu aşamada üç tip çelişki ayırt edilmektedir: iki gereksinimin birbirini geçersiz kıldığı doğrudan mantıksal zıtlık ("Kullanıcı şifresini sıfırlayabilmelidir" ile "Şifre değiştirme işlemine izin verilmemelidir" gibi), aynı işlemi farklı şekilde tanımlayan kuralların yarattığı kapsam çakışması ve örneğin parola sıfırlama akışının metin boyunca hiç tanımlanmaması gibi eksik kritik süreçler. Muğlak ifadeler ise ayrı bir analiz dalında ele alınır; "Sistem hızlı yanıt vermelidir" gibi cümleler, modelin önerdiği ölçülebilir kriterlerle "Sistem, kullanıcının bir form gönderdikten sonra 1 saniye içinde yanıt vermelidir." biçiminde yeniden yazılır. Üretilen her kayıt `severity` alanında `low` / `medium` / `high` etiketiyle raporlanır ve bu etiket bir sonraki katmanda kullanıcı hikâyesi önceliğini belirlemekte de kullanılır.
...
Analizin sonucunda elde edilen `AnalysisReport` nesnesi, üçüncü katmanda eş zamanlı olarak üç farklı çıktı formatına yönlendirilir. İlk çıktı, ISO/IEC/IEEE 29148 standardına uyumlu bir SRS belgesidir [4]; "Giriş", "Genel Açıklama", "Spesifik Gereksinimler" ve "Doğrulama" başlıkları analiz çıktısındaki alanlardan dinamik biçimde doldurulur. Belge, fpdf2 kütüphanesi üzerinden Türkçe karakter desteğine sahip bir PDF olarak üretilir; gerektiğinde python-docx ile DOCX formatında ikinci bir kopya da oluşturulabilir. Spesifik Gereksinimler bölümü, her bir `Requirement` için kimlik, tip, aktör, nesne ve metin alanlarını tablo biçiminde sıralar. İkinci çıktı, çevik takımlar için üretilen kullanıcı hikâyesi kartlarıdır. Her FR cümlesi "Bir [aktör] olarak, [eylem] yapmak istiyorum, böylece [fayda] elde ederim." şablonuna dönüştürülür; aktör alanı NER fazından, eylem alanı bağımlılık ayrıştırmasındaki kök fiilden, fayda alanı ise LLM tabanlı bir tamamlama adımından beslenir. Hikâyenin önceliği, ilgili çelişki veya gap kaydının `severity` değerine göre `HIGH` / `MEDIUM` / `LOW` olarak işaretlenir. Üçüncü çıktı, Gherkin formatında BDD senaryolarıdır; her kullanıcı hikâyesi en az bir Given/When/Then bloğuna açılır. Örneğin "Kullanıcı geçerli bilgilerle giriş yapar" hikâyesi `Given kullanıcı giriş sayfasında / When geçerli e-posta ve şifre girer / Then sistem 1 saniye içinde gösterge paneline yönlendirir` somut senaryosuna dönüşür. Bu çıktı, doğrudan otomasyon test çatılarına aktarılabilecek biçimdedir ve ölçülebilirlik kriterleri, ikinci katmanda üretilen iyileştirme kayıtlarından doğrudan beslenir; böylece üst aşamadaki analiz kalitesi alt aşamadaki test kalitesine kesintisiz biçimde aktarılmış olur.
